...
namespace Velixo.EBanking.BaiParsing
{
    public class BaiParser
    {
        ...
        public BaiFile Parse(byte[] input)
        {
            string content = System.Text.Encoding.UTF8.GetString(input);
            _data = content.Split(new[] { "\r\n", "\r", "\n" }, StringSplitOptions.None);
            return _Parse();
        }
        ...
        private BaiFile _Parse()
        {
            ...
            foreach (var data in _data.Select((value, index) => new { value, index }))
            {
                ...
                else if (line.StartsWith("99") && !trailerReached)
                { 
